...
export class AppLayout implements OnDestroy {
  ...
  readonly puedeVerActividad = computed(() => this.authService.roles().includes('ADMIN'));
  ...
  alternarPerfil(): void {
    ...
    if (!this.puedeVerActividad() && this.perfilTab() === 'actividad') {
      this.perfilTab.set('datos');
    }
  }

  seleccionarPerfilTab(tab: PerfilTab): void {
    if (tab === 'actividad' && !this.puedeVerActividad()) {
      this.perfilTab.set('datos');
      return;
    }
